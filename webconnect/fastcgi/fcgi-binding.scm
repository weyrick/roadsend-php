...
;    (macro FCGX_VFPrintF::int (FCGX_Stream* string va_list) "FCGX_VFPrintF")
    ...
    (type s-FCGX_Stream (struct (rdNext::uchar* "rdNext")
				(wrNext::uchar* "wrNext")
				(stop::uchar* "stop")
				(stopUnget::uchar* "stopUnget")
				(isReader::int "isReader")
				(isClosed::int "isClosed")
				(wasFCloseCalled::int "wasFCloseCalled")
				(FCGI_errno::int "FCGI_errno")
				(fillBuffProc::*s-FCGX_Stream*->void "fillBuffProc")
				(emptyBuffProc::*s-FCGX_Stream*,int->void "emptyBuffProc")
				(data::void* "data")) "struct FCGX_Stream")
...
;    (type void* (pointer void) "void *")
    ...
    (type s-FCGX_Request (struct (requestId::int "requestId")
				 (role::int "role")
				 (in::FCGX_Stream* "in")
				 (out::FCGX_Stream* "out")
				 (err::FCGX_Stream* "err")
				 (envp::string* "envp")
				 (paramsPtr::s-Params* "paramsPtr")
				 (ipcFd::int "ipcFd")
				 (isBeginProcessed::int "isBeginProcessed")
				 (keepConnection::int "keepConnection")
				 (appStatus::int "appStatus")
				 (nWriters::int "nWriters")
				 (flags::int "flags")
				 (listen_sock::int "listen_sock")) "struct FCGX_Request")
...
;     (macro FCGI_puts::int (string) "FCGI_puts")
     (macro FCGI_fprintf::int (FCGI_FILE* string . obj) "FCGI_fprintf")
