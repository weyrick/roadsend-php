...
;	   (apache-run-webapp::int areq::ApacheRequest* webapp-lib::string mount-point::string index-file::string)
   ...
   (extern
    (include "windows-apache.h")

    (type aptable (opaque) "table*")
    (type appool (opaque) "ap_pool*")
    ...
    ;this is the main hook in from the apache module
;    (export slub-apache-run-url "slub_run_url")
...
;		 (string-append " -DBIGLOO_MAIN=phpoo_initialize -DBIGLOO_EXIT='BUNSPEC,' "
;				*cc-options*))) )

; (define (slub-apache-run-url foo)
;    (fprint (current-error-port) "string is: " (mkstr foo))
;    4)
...
; 			       (ApacheUpload*-tempname upload)))
   ...
   (let loop ((cur-upload upload))
      (when (> *debug-level* 1)
	 (apache-log-message (format "processing upload: ~a" (mkstr (ApacheUpload*-filename cur-upload)))))
...
; called from mod_pcc
(define (apache-run-webapp::int areq::ApacheRequest* webapp-lib::string filename::string directory::string index-file::string)
...
(define (do-run-url areq filename webapp-lib index-file)
   (let ((req (ApacheRequest*-req areq)))
	  ...
	  (bind-exit (return)
		...
		("GET" (parse-get-vars areq))
		("POST" (parse-get-vars areq)
                        (parse-post-vars areq))
...
(define (parse-cookies req::request-rec*)
...
(define (parse-get-vars req::ApacheRequest*)
...
(define (parse-post-vars req::ApacheRequest*)
