...
/*
 * Roadsend PHP: adapted from apr_snprintf.c found in the
 * Apache Portable Runtime. The original header may be found
 * below.
 *
 * A few changes have been made to produce output compatible with Zend PHP
 *.
 */
...
#include <limits.h>
#include <string.h>
#include "pcc_snprintf.h"
 
typedef struct pcc_vformatter_buff_t pcc_vformatter_buff_t;
struct pcc_vformatter_buff_t {
    char *curpos;
    char *endpos;
};
...
typedef wide_int widest_int;
typedef u_wide_int u_widest_int;
...
static char *pcc_cvt(double arg, int ndigits, int *decpt, int *sign, 
                     int eflag, char *buf)
{
    register int r2;
    int mvl;
...
static char *pcc_ecvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (pcc_cvt(arg, ndigits, decpt, sign, 1, buf));
}

static char *pcc_fcvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (pcc_cvt(arg, ndigits, decpt, sign, 0, buf));
}

/*
 * pcc_gcvt  - Floating output conversion to
 * minimal length string
 */

static char *pcc_gcvt(double number, int ndigit, char *buf, boolean_e altform)
{
    int sign, decpt;
    register char *p1, *p2;
    register int i;
    ...
    p1 = pcc_ecvt(number, ndigit, &decpt, &sign, buf1);
    p2 = buf;
    if (sign)
        *p2++ = '-';
    ...
    char *buf, int *len)
{
    ...
    if (format == 'f')
        p = pcc_fcvt(num, precision, &decimal_point, is_negative, buf1);
    else /* either e or E format */
        p = pcc_ecvt(num, precision + 1, &decimal_point, is_negative, buf1);
 ...
 */
int pcc_vformatter(int (*flush_func)(pcc_vformatter_buff_t *),
    pcc_vformatter_buff_t *vbuff, const char *fmt, va_list ap)
{
    register char *sp;
    register char *bep;
    register int cc = 0;
    ...
    while (*fmt) {
	    ...
	    /*
		    ...
		    break;
                 ...
                 * * We use &num_buf[ 1 ], so that we have room for the sign
                 */
                s = pcc_gcvt(fp_num, precision, &num_buf[1],
                            alternate_form);
                if (*s == '-')
                    prefix_char = *s++;
                else if (print_sign)
...
static int snprintf_flush(pcc_vformatter_buff_t *vbuff)
{
    /* if the buffer fills we have to abort immediately, there is no way
     * to "flush" an pcc_snprintf... there's nowhere to flush it to.
     */
    return -1;
}


int pcc_snprintf(char *buf, pcc_size_t len, 
                                     const char *format, ...)
{
    int cc;
    va_list ap;
    pcc_vformatter_buff_t vbuff;
    ...
    va_start(ap, format);
    cc = pcc_vformatter(snprintf_flush, &vbuff, format, ap);
    va_end(ap);
    if (len != 0) {
        *vbuff.curpos = '\0';
    }
...
int pcc_vsnprintf(char *buf, pcc_size_t len, const char *format,
                               va_list ap)
{
    int cc;
    pcc_vformatter_buff_t vbuff;
