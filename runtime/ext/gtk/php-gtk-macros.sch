...
;; XXXXXX bigloo 2.9a hack XXXXXXX
;; segfaults on the (load) above so this is copied from php-macros.scm
...
(define-macro (defclass klass . properties)
   (let* ((classname (if (list? klass)
      ...
      `(add-startup-function-for-extension "gtk"
	(lambda ()
...
(define-macro (defenum name . pairs)
   `(begin
       ,@(map (lambda (enum-name)
		 `(defconstant ,enum-name
		     (convert-to-integer
		      (pragma::int ,(symbol->string enum-name)))))
	      (map cadr pairs))))

;    (let ((make-defconstant-form (let ((i 0))
; 					(lambda (symbol)
; 					   (begin0
; 					    `(defconstant ,symbol (convert-to-integer ,i))
; 					    (set! i (+ i 1)))))))
;       `(begin
; 	  ,@(map (lambda (pair) (make-defconstant-form (cadr pair))) pairs)
; 	  (define (,(symbol-append 'translate- name) x)
; 	     (let ((x (mkfixnum x)))
; 		(if (< -1 x ,(length pairs))
; 		    (car (list-ref ',pairs x))
; 		    #f))))))

(define-macro (defflags name . pairs)
   `(begin
       ,@(map (lambda (flag-name)
		 `(defconstant ,flag-name (convert-to-integer
					   (pragma::int ,(symbol->string flag-name)))))
	      (map cadr pairs))))

;    (let ((make-defconstant-form (let ((i #e1))
; 				   (lambda (symbol)
; 				      (begin0
; 				       `(defconstant ,symbol (convert-to-integer ,(*elong #e2 i)))
; 				       (set! i (+elong i #e1)))))))
;       `(begin
; 	  ,@(map (lambda (pair) (make-defconstant-form (cadr pair))) pairs)
; 	  (define (,(symbol-append 'translate- name) x)
; 	     (let ((x (onum->elong (convert-to-number x))))
; 		(let loop ((flags ',pairs) (matches '()))
; 		   (if (null? flags)
; 		       matches
; 		       (loop (cdr flags)
; 			     (let ((flag-val-elong (onum->elong (eval (cadar flags)))))
; 				(if (=elong flag-val-elong (bit-andelong x flag-val-elong))
; 				    (cons (caar flags) matches)
; 				    matches))))))))))
...
(define *defs-files* '("defs/gdk.defs" "defs/gtk.defs" "defs/gtk-extrafuncs.defs"
			 "defs/scintilla.defs" "defs/libglade.defs"))
...
(define (pragma-form return-type fname param-names . rest)
   (let ((fname (symbol->string fname))
	 (var-list (key-assoc :var-list rest '()))
	 (separate (key-assoc :parse-list rest #f)))
      (match-case (assoc return-type gtk->php-db)
	 ((?- none) `(begin ,(make-pragma fname #f param-names) NULL))
	 ((?- int) `(convert-to-integer ,(make-pragma fname 'int param-names)))
	 ((?- string) `(mkstr ,(make-pragma fname 'string param-names)))
	 ((?- string-array) `(string*->php-hash ,(make-pragma fname 'string* param-names)))
	 ((?- bool) `(convert-to-boolean ,(make-pragma fname 'bool param-names)))
	 ((?- double) `(convert-to-float ,(make-pragma fname 'double param-names)))
	 ((?- boxed ?canon-type)
	  `(gtk-wrapper-new ',canon-type ,(make-pragma fname (studly-to-dashes canon-type) param-names)))
	 ((?- object ?canon-type)
	  `(gtk-object-wrapper-new ',canon-type ,(make-pragma fname (studly-to-dashes canon-type) param-names)))
	 ((?- flags) `(convert-to-integer ,(make-pragma fname 'int param-names)))
	 ((?- enum) `(convert-to-integer ,(make-pragma fname 'int param-names)))
	 (?- (error 'pragma-form "patz" (cons* return-type fname param-names rest))))))
...
;		(print "registering struct type " c-type)
		(unless (assoc c-type gtk->php-db)
	       ...
	       ((enum ?type ??- (c-name ?c-type) . ?-)
		(unless (assoc c-type gtk->php-db)
		   (set! gtk->php-db
			 (cons (list c-type 'enum) gtk->php-db))))
	       ((flags ?type ??- (c-name ?c-type) . ?-)
		(unless (assoc c-type gtk->php-db)
		   (set! gtk->php-db
   ...
   `(begin
       ,@(map (lambda (method)
		 (try (begin
			 ...
			 (let* ((args (cons `($this :gtk-type ,klass)
					    (cdr method)))
				...
				(required-parameters (required-parameters (cdr args)))
				...
				(let ,(map param-cast-form
					   parameter-types parameter-names args)
				   ,@(if (eqv? method-name 'add)
					 `((debug-trace 3 "about to call " ',method-name " with args : "
							,@parameter-names))
					 '())
