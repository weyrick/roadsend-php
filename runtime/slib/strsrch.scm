(module strsrch
   ...
   (export (slib:string-index str chr)
	   (string-index-ci str chr)
	   (string-reverse-index str chr)
	   (string-reverse-index-ci str chr)
	   (substring? pat str)
	   (substring-ci? pat str)
	   (subskip skip pat patlen str strlen char=)
	   (subloop pat patlen str strlen char=)
	   (find-string-from-port? str <input-port> . max-no-char)
	   (string-subst text old new . rest)
	   (count-newlines str)))
...
(define (slib:string-index str chr)
...
(define (substring? pat str)
  ...
  (cond ((zero? patlen) 0)		; trivial match
	...
	;; use faster slib:string-index to match a single-character pattern
	((= 1 patlen) (slib:string-index str (string-ref pat 0)))
