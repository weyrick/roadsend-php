...
; (define format:symbol-case-conv (lambda (a) 'zot))
...
; (define format:iobj-case-conv (lambda (a) 'zot))
...
(define format:ascii-non-printable-charnames (zoot))
...
(define format:case-conversion::obj #f)
...
(define (format:error . args)		; never returns!
  (let ((error-continuation format:error-continuation)
	...
	(let ((format-string (cadr format-args)))
	  ...
	  (cl-format port "~%FORMAT: error with call: (format ~a \"~a<===~a\" ~
                                  ~{~a ~}===>~{~a ~})~%        "
		  (car format:args)
...
(define (format:iobj->str iobj)
  (if format:read-proof; or .. format:iobj-case-conv)
      (string-append
...
(define cl-format format:format)
